...
const UserDataChart = ({ data }) => {
  const usersData = {};
  data.forEach((userData) => {
    const dateString = format(
      new Date(userData.registration_date._seconds * 1000),
      "MM-dd-yyyy"
    );
    if (usersData[dateString]) {
      usersData[dateString] += 1;
    } else {
      usersData[dateString] = 1;
    }
  });

  const sortedLabels = Object.keys(usersData).sort((a, b) => {
    return (
